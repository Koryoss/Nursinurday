...
const RULES = [
  ...
  // ── SSOT §3 (CF-REG-2026-001) 금지어 보강 ────────────────────
  {
    id: 'CONFIRM-01',
    label: '확진 표현',
    re: /확진(입니다|됩니다|됐|되었|판정|받았)/,
    desc: 'CF-REG-2026-001 §1: 확진 표현 금지 (비의료기기)',
  },
  {
    id: 'CURE-01',
    label: '완치·치료효과 단정',
    re: /완치(됩니다|됐|되었|가능|될\s*수)|치료\s*효과(가\s*입증|를\s*입증|가\s*있)/,
    desc: 'CF-REG-2026-001 §1: 완치·치료효과 입증 표현 금지',
  },
  {
    id: 'WORSEN-01',
    label: '병적 악화 판정',
    re: /병적(\s*으로)?\s*악화|병적\s*(진행|변화|상태)|악화(되고\s*있습니다|된\s*상태입니다)/,
    desc: 'CF-REG-2026-001 §1: 병적 악화 판정 금지 (관찰/질문형만 허용)',
  },
  {
    id: 'SEIZURE-01',
    label: '발작 예측',
    re: /발작(을|이)?\s*(예측|예상|일어날\s*것|올\s*것|발생할\s*것)|발작\s*(위험|가능성이\s*높)/,
    desc: 'CF-REG-2026-001 §1: 발작 예측 금지',
  },
  {
    id: 'RISKSTATE-01',
    label: '위험 상태 단정',
    re: /위험(한)?\s*상태(입니다|예요|이에요|로\s*보|에\s*있)|위험\s*수준(입니다|이에요)/,
    desc: 'CF-REG-2026-001 §1: 위험 상태 단정 금지 (위기감지는 1393 연계로 처리)',
  },
]
