...
export function bandFromZ(value: number | null | undefined, baseline?: Baseline): Band {
  if (value === null || value === undefined || !baseline || baseline.rolling7_sd === 0) {
    return 'normal'
  }
...
export function buildCorrelations(points: TrendPoint[]): CorrelationItem[] {
  ...
  return pairs.map(([left, right, pair]) => {
    ...
    let direction: CorrelationItem['direction'] = 'unclear'
    let label = '뚜렷한 연관은 아직 관찰되지 않았어요.'

    if (coefficient !== null && coefficient > 0.3) {
      direction = 'together'
      label = '함께 오르내리는 흐름이 관찰돼요.'
    } else if (coefficient !== null && coefficient < -0.3) {
      direction = 'opposite'
      label = '서로 반대로 움직이는 흐름이 관찰돼요.'
    }
